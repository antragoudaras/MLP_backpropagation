...
def confusion_matrix(predictions, targets):
    ...
    one_hot_predictions = np.eye(n_classes)[np.argmax(predictions, axis=1)] 
...
def confusion_matrix_to_metrics(confusion_matrix, beta=1.):
    ...
    metrics[f'f{beta}_beta'] = ((1 + beta**2) * (metrics['precision'] * metrics['recall'])) / (beta**2 * metrics['precision'] + metrics['recall'])
    return metrics
...
def evaluate_model(model, data_loader, num_classes=10):
    ...
    for _ in tqdm(range(len(data_loader))):
        ...
        with torch.no_grad():
            ...
            labels = labels.cpu()
            conf_mat_per_batch = confusion_matrix(predictions, labels)
    ...
    plot_confusion_matrix(total_conf_mat, 'torch')
...
def train(hidden_dims, lr, use_batch_norm, batch_size, epochs, seed, data_dir):
    ...
    Returns:
      ...
      test_accuracy: scalar float, average accuracy on the test datasetconfusion_matrix of the model that 
    ...
    n_inputs = images.view(images.shape[0], -1).shape[1]
    n_classes = 10
    ...
    for epoch in tqdm(range(epochs)):
        ...
        for batch in range(len(cifar10_loader['train'])):
            ...
            train_preds += (predictions.argmax(dim=1) == labels).sum().item()
            count += labels.shape[0]
            train_accuracy_per_batch[f'batch_{batch}'] = (predictions.argmax(dim=1) == labels).sum().item() / labels.shape[0]
            
            train_loss_ += loss.item()
            train_loss_per_batch[f'batch_{batch}'] = loss.item()
        ...
        for batch in range(len(cifar10_loader['validation'])):
            ...
            with torch.no_grad():
                ...
                val_preds += (predictions.argmax(dim=1) == labels).sum().item()
                count += labels.shape[0]
                val_accuracy_per_batch[f'batch_{batch}'] = (predictions.argmax(dim=1) == labels).sum().item() / labels.shape[0]

                val_loss_ += loss.item()
                val_loss_per_batch[f'batch_{batch}'] = loss.item()
    ...
    print(f'Class {np.argmax(metrics["precision"]) + 1} has the highest precision of: {np.max(metrics["precision"])}')
    print(f'Class {np.argmax(metrics["recall"]) + 1} has the highest recall of: {np.max(metrics["recall"])}')
...
if __name__ == '__main__':
    ...
    def plot_loss(logging_info):
        ...
        if not os.path.exists(os.path.join(folder_path, 'loss_curves_torch.png')):
            plt.savefig(os.path.join(folder_path, 'loss_curves_torch.png'))
    ...
    def plot_accuracy(logging_info):
        ...
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        if not os.path.exists(os.path.join(folder_path, 'acc_curves_torch.png')):
            plt.savefig(os.path.join(folder_path, 'acc_curves_torch.png'))
